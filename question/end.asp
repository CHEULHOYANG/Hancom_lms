...
window.open('poll.asp?idx='+ idx,'quiz_window','fullscreen=yes,type=FullWindows,scrollbars=yes,top=0,left=0');
<%else%>
alert('로그인후 서비스를 이용해주세요!!');
<%end if%>
} 
</script>
    ...
    <div class="content">
        ...
        <div class="scont">
            <table class="btbl" style="width:830px">
                    ...
                    <tbody>
<%
sql = "select count(idx) as reccount from question_list where DateDiff(dd, getdate(), date2) <= 0"
set rs=db.execute(sql)
recordcount =rs(0)
rs.close

pagecount=int((recordcount-1)/pagesize)+1
sql = "SELECT top " & pagesize & " idx,title,date1,date2,price,(select count(idx) from question_mast where ca = A.idx) FROM question_list A where DateDiff(dd, getdate(), date2) <= 0 and idx not in"
sql = sql & "(select top " & ((page -1 ) * pagesize) & " idx from question_list where DateDiff(dd, getdate(), date2) <= 0 order by idx desc ) order by idx desc"
set rs=db.execute(sql)
