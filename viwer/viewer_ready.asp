...
if isUsr then %><!-- #include file = "../include/dbcon.asp" --><%
	...
	if cntPerm > 0 then	''프리미엄 수강
		...
		sql = "select top 1 tabidx from order_mast where  tabidx=" & Lecturaidx & " and id='" & str_User_ID & "' and buygbn = 1 and DateDiff(day,GetDate(),sday) <=0 and DateDiff(day,GetDate(),eday) >= 0 and state=0 and holdgbn=0"
		set dr = db.execute(sql)
...
if isViewpg then 
	...
	If gbn= "mp4" Or gbn="wmv" Then
		response.redirect "/yplayer/index.asp?order_mast_idx="& request("order_mast_idx") &"&plidx="& plidx &"&gu="& gbn &""
	End If
	If gbn= "free" Then
