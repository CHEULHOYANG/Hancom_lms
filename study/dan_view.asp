...
<%If precheck = 1 Or dancheck = 1 Then
                        ...
                        <tr>
                            <th>구성</th>
                            <td colspan="3"><input type="checkbox" name="II" id="II" value="<%=idx%>" checked/>&nbsp;<strong><font color='#000000'>[강좌] <%=strnm%></font></strong>&nbsp;&nbsp;<strong class="fr"><%=formatnumber(intprice,0)%></strong>원<%
...
%><br /><input type="checkbox" name="JJ" id="JJ" value="<%=rs(2)%>" checked/>&nbsp;<strong>[교재] <a href="/book/content.asp?idx=<%=rs(2)%>" target="_blank"><%=rs(1)%></a></strong>&nbsp;&nbsp;<strong class="fr"><%=formatnumber(rs(0),0)%></strong>원<%
				rs.close
				End if
		Next
	End if
...
<input type="hidden" name="jj_count" value="<%=bcount%>">
			...
			<div class="rbtn">
				<a href="dan_list.asp?ca1=<%=ca1%>&ca2=<%=ca2%>" class="mbtn">목록으로</a>
				<%If precheck = 1 Or dancheck = 1 Then%>
<%If precheck = 1 Then%>
<%
...
<%End if%>
<%If dancheck = 1 Then%>
				<a href="/my/01_buy_dan_view.asp?idx=<%=idx%>" class="mbtn red">강의보기</a>
<%End if%>
				<%else%>
				<a href="javascript:AryDanBuy(<%=strProg%>,<%=bcount%>);" class="mbtn red">수강신청</a><a href="javascript:AryDanBaguy(<%=strProg%>,<%=bcount%>);" class="mbtn grey">장바구니</a>
				<%End if%>
			</div>
...
If rs(0) = 0 then
	...
	response.write"<a href=""javascript:go2Logpg1("& strProg &",'reply_write.asp?idx="& request("idx") &"&dntGbn="& request("dntGbn") &"&repage=dan_view');"" class=""mbtn red"">수강후기작성</a>"
