...
Dim title,writer,content,wrtid,image2,pwd
...
sql = "select title,writer,content,wrtid,image2,pwd from board_board where idx=" & idx
set dr = db.execute(sql)
...
<input type="hidden" name="strSearch" value="<%=strSearch%>">
            ...
            <table class="ftbl" style="width:830px">
                ...
                <tbody>
					...
					<tr>
						<th>첨부파일</th>
						<td><input name="filenm" type="file" id="filenm" class="inptxt1 w300"><p style="margin:5px 0" class="stip"><%If Len(image2)> 0 then%><%=image2%>&nbsp;<%End if%>* doc,hwp,zip,ppt,txt,xls,pdf,alz,pptx,xlsx,docx 5MB이하만 가능</p></td>		
					</tr>	
                    <tr>
                        <th>비밀번호</th>
