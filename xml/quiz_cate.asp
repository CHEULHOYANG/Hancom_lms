...
<%
sql="select idx,title from quiz_category where deep=1 and uidx="& idx &" order by ordnum asc,idx desc"
...
do until rs.eof
%>
      <option value="<%=rs(0)%>" <%If Int(rs(0)) = Int(ca2) Then response.write"selected" End if%>><%=rs(1)%></option>
<%
rs.movenext
Loop
rs.close
