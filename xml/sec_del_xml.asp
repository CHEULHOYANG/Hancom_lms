...
<%
	...
	totalnum = 0
	for jj = 0 to UBound(keyAry) - 1

		db.execute("delete sectionTab where idx=" & keyAry(jj))

	Next

	set objFso = Nothing

	db.execute("update LecturTab set totalnum=totalnum - " & totalnum & " where idx=" & key)

	sql = "select idx from sectionTab where l_idx=" & key & " order by ordnum"
	set dr = db.execute(sql)

	if Not dr.Bof or Not dr.Eof then
		kk = 1
		do until dr.Eof
			db.execute("update sectionTab set ordnum=" & kk & " where idx=" & dr(0))
		dr.moveNext
		kk = kk + 1
		Loop
	end if
	...
	sql ="select idx,strnm,ordnum,strtime,lecsum,lecsrc,freegbn from sectionTab where l_idx=" & key & " order by ordnum"
	set dr = db.execute(sql)
...
%>
						<tr>
							...
							<td><img src="../rad_img/a_up.gif" style="cursor:pointer;" onClick="setOrder('up','<%=isCols(0)%>',<%=isCols(2)%>,'<%=maxCnt%>','<%=key%>');">
							<img src="../rad_img/a_down.gif" style="cursor:pointer;" onClick="setOrder('dwn','<%=isCols(0)%>',<%=isCols(2)%>,'<%=maxCnt%>','<%=key%>');"></td>
							<td><%=isCols(2)%>회</td>
							...
							<td><input type="checkbox"<% if isCols(6) = "1" then response.write " checked" %> value="<%=isCols(0)%>" onClick="setFreeMove(this,'<%=key%>');"></td>
						</tr>
<% Next %>
			</tbody>
		</table>
