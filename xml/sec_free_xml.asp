...
<%
	...
	sql = "update sectionTab set freegbn=" & keyAry(1) & " where idx=" & keyAry(0)
	db.execute(sql)

	sql ="select idx,strnm,ordnum,strtime,lecsum,lecsrc,freegbn from sectionTab where l_idx=" & keyAry(2) & " order by ordnum"
...
%>
						<tr>
							...
							<td><img src="../rad_img/a_up.gif" style="cursor:pointer;" onClick="setOrder('up','<%=isCols(0)%>',<%=isCols(2)%>,'<%=maxCnt%>','<%=keyAry(2)%>');">
							<img src="../rad_img/a_down.gif" style="cursor:pointer;" onClick="setOrder('dwn','<%=isCols(0)%>',<%=isCols(2)%>,'<%=maxCnt%>','<%=keyAry(2)%>');"></td>
							<td><%=isCols(2)%>회</td>
							...
							<td><input type="checkbox"<% if isCols(6) = "1" then response.write " checked" %> value="<%=isCols(0)%>" onClick="setFreeMove(this,'<%=keyAry(2)%>');"></td>
						</tr>
<% Next %>
			</tbody>
		</table>
