...
sql = "select count(idx) from bookmark_mast where id = '"& cnum(1) &"' and v_idx = '"& cnum(0) &"'"
...
Else
	...
	sql = "insert into bookmark_mast (id,v_idx,vtime)values"
	sql = sql & "('" & cnum(1) & "'"
	sql = sql & ","& cnum(0)
	sql = sql & ",'"& Int(cnum(2)) &"'"
	sql = sql & ")"
...
sql = "select vtime,idx from bookmark_mast where v_idx = "& cnum(0) &" and id = '"& cnum(1) &"'"
set rs=db.execute(Sql)
