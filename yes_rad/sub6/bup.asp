...
If gbn = 1 then
	sql = "select idx,ordnum from board_mast where ordnum < '"& now_order &"' and ygbn = 1 order by ordnum asc"
Else
	sql = "select idx,ordnum from board_mast where ordnum < '"& now_order &"' and (ygbn = 2 or ygbn = 3) order by ordnum asc"
End if
