...
sub Manage()
	...
	'일별 카운터를 구한다
	sql = "select vDD,count(vNum) from "&theTable&sqlValue&" group by vDD order by vDD"
	set rs = db.execute(sql)
...
dim color_bar : color_bar = "#76A7FA,#CC0000,#FF6633,#76A7FA,#C5A5CF,#BC5679,#b87333,#cc9966,#009999,#cc99cc,#669933,#76A7FA,#CC0000,#FF6633,#76A7FA,#C5A5CF,#BC5679,#b87333,#cc9966,#009999,#cc99cc,#669933#76A7FA,#CC0000,#FF6633,#76A7FA,#C5A5CF,#BC5679,#b87333,#cc9966,#009999,#cc99cc,#669933,#76A7FA,#CC0000,#FF6633,#76A7FA,#C5A5CF,#BC5679,#b87333,#cc9966,#009999,#cc99cc,#669933,"
...
<div class="container">
	...
	<div class="content">
		<h2 class="cTit"><span class="bullet"></span>일별통계</h2>
...
<%for i = 1 to dataLength : SetDetail(i)%>	
        ['<%=i%>일 (<%=round(arrData(i,2),1)%>%)', <%=arrData(i,0)%>, 'color: <%=color_bar(i)%>'],
<%next%>		
