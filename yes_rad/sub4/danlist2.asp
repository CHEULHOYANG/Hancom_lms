...
<div class="container">
	...
	<div class="content">
		<h2 class="cTit"><span class="bullet"></span>선생님별매출통계</h2>

		<div class="tbl_top">
			<form name="form" method="post" action="danlist2.asp"><select name="tid" onChange="document.form.submit();" class="seltxt w200">
			<option<% if Len(tid) = 0 then response.write " selected"%> value="">선생님선택</option>
<%
sql = "select id,name,guk from teach_mast order by name asc,idx desc"
...
Do Until rs.eof
%>
										<option<% if rs(0) = tid then response.write " selected"%> value="<%=rs(0)%>">[<%=rs(2)%>] <%=rs(0)%>(<%=rs(1)%>)</option>
<%
rs.movenext
Loop
rs.close
...
<% if isRecod then
						...
						<tr>
							<td colspan="3"><%=yy%>년 <%=mm%>월 단과 매출 없습니다.</td>
						</tr><% end if %>
			</tbody>
		</table>
<%else%>
<p style="text-align:center;margin:30px 0"><font color='#cc0000'>선생님을 선택해주세요.</font></p>
<%End if%>
