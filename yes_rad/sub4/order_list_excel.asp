...
Response.AddHeader "Content-Disposition","attachment;filename="& left(now(),10) &"[수강자목록].xls"
...
Dim strClmn : strClmn = " idx,id + '(' + dbo.MemberNm(id) + ')',titlen = title + dbo.LectuTitle(tabidx,buygbn),sday,dbo.PayTypeStr(paytype),payday,state,eday "
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#DEDFDE">
						<tr bgcolor="#EEEEEE" height="30" align="center">
							<td width="5%" height="30"><strong><font color="#000000">No.</font></strong></td>
							<td width="35%" height="30"><strong><font color="#000000">상품명</font></strong></td>
							<td width="16%" height="30"><strong><font color="#000000">구매자ID(이름)</font></strong></td>
							<td width="16%" height="30"><strong><font color="#000000">결제방법</font></strong></td>
							<td width="13%" height="30"><strong><font color="#000000">결제일</font></strong></td>
							<td width="15%" height="30"><strong><font color="#000000">만료일</font></strong></td>
						  </tr><% 
						  if isRecod then
				set dr = db.execute(sql)
				isRows = split(dr.GetString(2),chr(13))
						...
						<tr height="25" bgcolor="#FFFFFF" align="left">
							...
							<td height="25">&nbsp; <%=cutStr(isCols(2),100)%></td>
							<td align="center" height="25"><%=isCols(1)%></td>
							<td align="center" height="25"><%=isCols(4)%>(<%if isCols(6)=0 then response.write"결제완료" else response.write"미결제" end if%>)</td>
							<td align="center" height="25"><%=formatdatetime(isCols(3),2)%></td>
							<td align="center" height="25"><%=formatdatetime(isCols(7),2)%></td>
						  </tr><% lyno = lyno - 1
						Next 
						else
						end if
