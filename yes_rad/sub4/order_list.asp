...
function order_end_input(idxnm){
	var delok = confirm("수료증을 등록을 하시겠습니까?");
	if(delok){
...
<div class="container">
	...
	<div class="content">
		<h2 class="cTit"><span class="bullet"></span>수강자목록</h2>
		...
		<div class="schWrap">
			...
			<div class="sch_area" style="line-height:40px">
				...
				<select name="mst_idx" class="seltxt" style="width:300px;">
					<option value="0" <%if mst_idx = 0 then response.write"selected" end if%>>전체 패키지</option>
<%
...
%>
					...
					<option value="0" <%if dan_idx = 0 then response.write"selected" end if%>>전체 단과</option>
<%
...
<input type="hidden" name="dan_idx" value="<%=dan_idx%>">
			...
			<span class="tbl_total">전체 <%=recordcount%>건 (<%=intpg%>page/<%=pagecount%>pages)&nbsp;<a href="order_list_excel.asp?intpg=<%=intpg%>&gbnS=<%=request("gbnS")%>&strsday=<%=request("strsday")%>&streday=<%=request("streday")%>&strPart=<%=request("strPart")%>&strSearch=<%=request("strSearch")%>&mst_idx=<%=mst_idx%>&dan_idx=<%=dan_idx%>" class="sbtn">엑셀로저장하기</a></span>
		</div>
		...
		<table class="tbl" style="width:100%">
			...
			<% for ii = 0 to UBound(isRows) - 1
						...
						<tr>
							...
							<td><%=isCols(4)%>(<%if isCols(6)=0 then response.write"결제완료" else response.write"미결제" end if%>)</td>
							<td><%=formatdatetime(isCols(3),2)%></td>
							<td><form name="form<%=isCols(0)%>" method="post" action="order_list_eday.asp?idx=<%=isCols(0)%>&intpg=<%=intpg%>&<%=varPage%>"><input name="eday" id="ed<%=isCols(0)%>" value="<%=formatdatetime(isCols(7),2)%>" readonly class="inptxt1 w80" />&nbsp;<a href="javascript:document.form<%=isCols(0)%>.submit();" class="fbtn">수정</a></form></td>
							<td><a href="javascript:order_end_input(<%=isCols(0)%>);" class="btns trans">발급</a></td>
							<td><a href="javascript:go2Del('<%=isCols(0)%>');" class="btns trans">삭제</a></td>
						</tr>
