...
sql = "select jemok,neyong from faqTab where idx=" & idx
...
<form name="regfm" action="fedit_ok.asp" method="post" style="display:inline;"> 
...
<input type="hidden" name="strSearch" value="<%=request("strSearch")%>">    
		<table class="ftbl mb50" style="width:100%">
				...
				<tbody>
					<tr>
						<th>제목</th>
						<td><input type="text"  name="jemok" class="inptxt1 w400" value="<%=jemok%>"></td>
					</tr>
					<tr>
						<th>내용</th>
		...
		<div class="rbtn">
			...
			<a href="fneyong.asp?idx=<%=idx%>&intpg=<%=intpg%>&strPart=<%=request("strPart")%>&strSearch=<%=request("strSearch")%>" class="btn trans">목록보기</a>		
		</div>
