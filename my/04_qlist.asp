...
if Len(Request("searchstr")) = 0 Then
	...
	sql = "select Count(qidx) from " & tabnm &" where quserid='" & str_User_ID & "'"
	...
	if recordcount > 0 then
		...
		ssqll = "select  top " & pagesize & strClmn & "from " & tabnm & " where quserid='" & str_User_ID & "' and qidx not in (select top " & ((intpg -1 ) * pagesize) & " qidx from " & tabnm & " where quserid='" & str_User_ID & "' order by qidx desc) order by qidx desc"
	end If
...
Else
	...
	dim query
	If request("searchpart") = "0" then	query = " quserid='" & str_User_ID & "' and qtitle like '%" & Replace(request("searchstr"),"'","''") & "%'"
	If request("searchpart") = "1" then	query = " quserid='" & str_User_ID & "' and qcont like '%" & Replace(request("searchstr"),"'","''") & "%'"
	If request("searchpart") = "2" then	query = " quserid='" & str_User_ID & "' and (qtitle like '%" & Replace(request("searchstr"),"'","''") & "%' or qcont like '%" & Replace(request("searchstr"),"'","''") & "%')"
...
function go2Neyong(idxn,vgbn){
	...
	if(vnum < 1){
		location.href="04_qneyong.asp?idx=" + idxn + "&intpg=<%=intpg%>&<%=varPage%>";
	}else{
		if(vnum == 1){
...
<%End if%>
		...
		<div class="cbtn"> <a href="04_qwrite.asp" class="mbtn grey">문의하기</a> <a href="04_qlist.asp" class="mbtn">목록으로</a> </div>
