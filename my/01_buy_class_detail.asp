...
function go2ViewerPlay(flg,intUnm,keynm){
	...
	var urlnm="/viwer/viewer_ready1.asp?gbn=" + keynm + "&plidx=" + intUnm;
...
function go2ViewerPlay_n(flg,intUnm,keynm){
	...
	var urlnm="../viwer/viewer_ready1_n.asp?order_mast_idx=<%=request("order_mast_idx")%>&gbn=" + keynm + "&plidx=" + intUnm;
	var sFeatures = "width=980,height=560,top=0,left=0,scrollbars=no,resizable=no,titlebar=no";
	var k = window.open(urlnm, "viewpg", sFeatures);
	k.focus();
}
...
{ 
window.open('../quiz/quiz.asp?gu=class&idx='+ idx,'quiz_window','height=100,width=100,scrollbars=yes');
} 
</script>
...
End If
%>
				<a href="01_buy_class_view.asp?idx=<%=request("idx")%>" class="mbtn">돌아가기</a> <%
sql = "select count(idx) from lec_reply where vidx = "& lidx &" and id = '"& str_User_ID &"'"
Set rs=db.execute(sql)

If rs(0) = 0 then

	response.write"<a href=""reply_write.asp?idx="& request("idx") &"&lidx="& request("lidx") &"&repage=01_buy_class_detail"" class=""mbtn red"">수강후기</a>"
