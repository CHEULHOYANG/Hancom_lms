...
Dim strClmn : strClmn = "idx,title,tabnm,regdate,dbo.board_mast_title(tabnm),dbo.board_mast_ygbn(tabnm)"
...
if Len(Request("searchstr")) = 0 Then
	...
	sql = "select Count(idx) from " & tabnm &" where wrtid='" & str_User_ID & "'"
	...
	if recordcount > 0 then
		...
		ssqll = "select  top " & pagesize & strClmn & "from " & tabnm & " where wrtid='" & str_User_ID & "' and idx not in (select top " & ((intpg -1 ) * pagesize) & " idx from " & tabnm & " where wrtid='" & str_User_ID & "' order by idx desc) order by idx desc"
	end If
...
Else
	...
	dim query
	If request("searchpart") = "0" then	query = " wrtid='" & str_User_ID & "' and title like '%" & Replace(request("searchstr"),"'","''") & "%'"
...
%>
				<tr>
					<td><%=lyno%></td>
					<td><a href="/<%If isCols(5) = 1 Then Response.write"jalyo" Else Response.write"cumm" End if%>/list.asp?tabnm=<%=isCols(2)%>"><%=isCols(4)%></a></td>
					<td class="tl"><a href="/<%If isCols(5) = 1 Then Response.write"jalyo" Else Response.write"cumm" End if%>/content.asp?tabnm=<%=isCols(2)%>&idx=<%=isCols(0)%>"><%=isCols(1)%></a></td>
